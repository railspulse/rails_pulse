    ...
    module Cards
      class RequestsOverThreshold
        ...
        def to_metric_card
          ...
          total_requests = current_metrics[:total_requests] + previous_metrics[:total_requests]
          overall_percentage = total_requests > 0 ? (total_over.to_f / total_requests * 100).round(1) : 0

          # Calculate trend
          current_percentage = current_metrics[:total_requests] > 0 ?
            (current_metrics[:total_over].to_f / current_metrics[:total_requests] * 100) : 0
          previous_percentage = previous_metrics[:total_requests] > 0 ?
            (previous_metrics[:total_over].to_f / previous_metrics[:total_requests] * 100) : 0
          ...
          (start_day..end_day).each do |day|
            ...
            day_summaries.each do |summary|
              ...
              over_count = estimate_requests_over_threshold(summary, threshold)
              total_over_day += over_count
              total_requests_day += count
            end

            percentage = total_requests_day > 0 ?
              (total_over_day.to_f / total_requests_day * 100).round(1) : 0
          ...
          {
            id: "requests_over_threshold",
            context: "routes",
            title: "Requests Over Threshold",
        ...
        def calculate_period_metrics(summaries, start_time, end_time, threshold)
          ...
          { total_over: total_over, total_requests: total_requests }
        end

        # Estimate number of requests over threshold using percentile data
        # This is an approximation based on available percentile information
        def estimate_requests_over_threshold(summary, threshold)
          count = summary.count || 0
          return 0 if count == 0
