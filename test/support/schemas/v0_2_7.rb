# Schema snapshot from v0.2.7
# Used by migration regression tests to simulate an upgrade from that release.
# Missing vs current: p95/p99 on jobs, diagnostic fields on operations,
# response_size_bytes on requests, deployments table, actual_sql on operations.
module RailsPulse
  module TestSchemas
    V027 = lambda do |connection|
      ...
      unless connection.table_exists?(:rails_pulse_requests)
        connection.create_table :rails_pulse_requests do |t|
          ...
          t.text :tags
          # NOTE: response_size_bytes is intentionally absent — added by AddDiagnosticFields migration
          t.timestamps
        end
      ...
      unless connection.table_exists?(:rails_pulse_jobs)
        connection.create_table :rails_pulse_jobs do |t|
          ...
          # NOTE: p95_duration and p99_duration are intentionally absent — added by AddP95P99DurationToRailsPulseJobs
          t.text :tags
          t.timestamps
        end
      ...
      unless connection.table_exists?(:rails_pulse_operations)
        connection.create_table :rails_pulse_operations do |t|
          ...
          t.timestamp :occurred_at, null: false
          # NOTE: row_count, cache_hit, repeated_query_group, repetition_count, actual_sql
          #       are intentionally absent — added by AddDiagnosticFields and AddActualQueryToOperations
          t.timestamps
        end
