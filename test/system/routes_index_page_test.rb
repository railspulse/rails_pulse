...
class RoutesIndexPageTest < SharedIndexPageTest
  ...
  def test_column_selection_filters_table_and_persists_sorting
    ...
    assert_selector "#average_response_times_chart[data-chart-rendered='true']", wait: 10

    # Get initial table data count
    initial_row_count = all("table tbody tr").count
    assert initial_row_count > 0, "Should have table data to test with"

    # Sort the table by Average Response Time (descending should be default)
    within("table thead") do
      click_link "Average Response Time"
    end
    assert_selector "table tbody tr", wait: 3

    # Get the sorted order to verify it persists later
    sorted_rows = all("table tbody tr").map(&:text)

    # Verify URL contains sort parameter
    current_url = page.current_url
    assert current_url.include?("q%5Bs%5D="), "URL should contain sort parameter after clicking sort link"

    # Click on a chart column to select it
    page.execute_script(<<~JS)
      var chart = window.RailsCharts.charts["average_response_times_chart"];
      if (chart) {
        var option = chart.getOption();
        var seriesData = option.series[0].data;
        var xAxisData = option.xAxis[0].data;

        // Find the first column with non-zero data
        var dataIndex = 0;
        ...
        // Trigger our custom click event handler directly
        var params = {
          dataIndex: dataIndex,
          seriesIndex: 0,
        ...
        // Find the index controller and call handleColumnClick directly
        var indexElement = document.querySelector('[data-controller="rails-pulse--index"]');
        if (indexElement) {
          var controller = window.Stimulus.getControllerForElementAndIdentifier(indexElement, "rails-pulse--index");
          if (controller) {
    ...
    # Wait for the turbo frame to update
    sleep 1

    # Verify URL now contains selected_column_time parameter
    updated_url = page.current_url
    assert updated_url.include?("selected_column_time="), "URL should contain selected_column_time parameter after column selection"

    # Verify sort parameter is still present
    assert updated_url.include?("q%5Bs%5D="), "Sort parameter should persist after column selection"

    # Verify table data has changed (should be filtered)
    filtered_row_count = all("table tbody tr").count
    # The filtered results should typically be different from the initial count
    # (could be more or less depending on the data distribution)

    # Most importantly, verify the sort order is maintained
    current_sorted_rows = all("table tbody tr").map(&:text)

    # If we have the same routes after filtering, the sort order should be maintained
    # If different routes, we can't directly compare but the sort parameter presence confirms it worked
    if current_sorted_rows.length == sorted_rows.length && (current_sorted_rows & sorted_rows).length > 0
      # If we have overlapping data, the relative order of common items should be maintained
    ...
    # Verify chart column is visually highlighted (one column should be normal color, others gray)
    highlighted_columns = page.execute_script(<<~JS)
      if (window.RailsCharts && window.RailsCharts.charts && window.RailsCharts.charts['average_response_times_chart']) {
        return window.RailsCharts.charts['average_response_times_chart'].getOption().series[0].data.length;
      }
      return 0;
    JS

    # Verify we have chart data (simplified test due to JavaScript restrictions)
    assert highlighted_columns > 1, "Should have multiple chart columns"

    # Verify that the column selection functionality is working by checking URL contains the parameter
    current_url = page.current_url
