...
module RailsPulse
  class OperationQueryNormalizationTest < ActiveSupport::TestCase
    ...
    test "operation delegates normalization to SqlQueryNormalizer service" do
      query = "SELECT users.* FROM users WHERE users.id = 123"
      expected = "SELECT users.* FROM users WHERE users.id = ?"

      # Test that operation uses the service
      result = @operation.send(:normalize_query_label, query)
      assert_equal expected, result

      # Test that it's the same as calling the service directly
      service_result = RailsPulse::SqlQueryNormalizer.normalize(query)
      assert_equal service_result, result
    end
