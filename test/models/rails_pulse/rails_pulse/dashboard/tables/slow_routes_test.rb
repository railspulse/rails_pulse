...
class RailsPulse::Dashboard::Tables::SlowRoutesTest < ActiveSupport::TestCase
  def setup
    ENV["TEST_TYPE"] = "unit"
  ...
  test "to_table_data should return empty array when no data exists" do
    slow_routes = RailsPulse::Dashboard::Tables::SlowRoutes.new
    result = slow_routes.to_table_data

    assert_equal [], result
  end

  test "to_table_data should return routes sorted by average response time (slowest first)" do
    # Create routes
    ...
    # Should be ordered slowest first (opposite of FastRoutes)
    assert_equal "/api/slow", result[0][:route_path]
    assert_equal "/api/medium", result[1][:route_path]
    assert_equal "/api/fast", result[2][:route_path]

    # Check average response times
    assert_equal 200, result[0][:this_week_avg]
    assert_equal 100, result[1][:this_week_avg]
    assert_equal 50, result[2][:this_week_avg]
  end
  ...
  test "to_table_data should limit results to top 5 routes" do
    ...
    # Should only return top 5 routes (slowest first)
    assert_equal 5, result.length
    assert_equal "/api/route6", result[0][:route_path] # Slowest (70ms)
    assert_equal "/api/route2", result[4][:route_path] # 5th slowest (30ms)
  end
  ...
  test "to_table_data should handle routes with no last week data" do
    ...
    assert_equal 120, result[0][:this_week_avg]
  ...
  test "to_table_data should calculate trend correctly" do
    ...
    [
      ...
      [ stable_route, last_week_start + 2.days, 200.0 ],
      [ stable_route, this_week_start + 2.days, 200.0 ], # 0% (stable)
      [ worse_route, last_week_start + 2.days, 200.0 ],
      [ worse_route, this_week_start + 2.days, 400.0 ] # +100% (worse)
  ...
  test "to_table_data should include request count" do
    ...
    assert_equal 1, result.length
    assert_equal 5, result[0][:request_count]
  end
  ...
  test "to_table_data should have same structure as FastRoutes but different sorting" do
    # Create identical data as FastRoutes test
    fast_route = RailsPulse::Route.create!(method: "GET", path: "/api/fast")
    slow_route = RailsPulse::Route.create!(method: "GET", path: "/api/slow")
    ...
    # Verify structure
    result.each do |route_data|
      assert_includes route_data.keys, :route_path
      assert_includes route_data.keys, :this_week_avg
      assert_includes route_data.keys, :last_week_avg
      assert_includes route_data.keys, :percentage_change
      assert_includes route_data.keys, :request_count
      assert_includes route_data.keys, :trend
    end

    # Verify reverse ordering compared to FastRoutes
    assert_equal "/api/slow", result[0][:route_path] # Slowest first
    assert_equal "/api/fast", result[1][:route_path] # Fastest last
  end
end
